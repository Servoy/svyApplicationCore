/*
 * Moved this scope here from svyUtils as we don't want to open source this as part of svyUtils in it's current state
 */

/**
 * @private 
 * @properties={typeid:35,uuid:"675CA1A3-D632-4CC7-AD36-0ACF14B4A525",variableType:-4}
 */
var log = scopes.svyLogManager.getLogger('com.servoy.bap.utils.css')
...
function getTextWidth(font, text) {
	...
	try {
		...
		log.error("Error getting text width for font \"" + font + "\"", e);
		return 0;
	}
}
...
function getTextHeight(font) {
	...
	try {
		...
		log.error("Error getting text height for font \"" + font + "\"", e);
		return 0;
	} 
}
...
function StyleParser(styleName) {
	...
	if (!(this instanceof StyleParser)) {
		log.warn("scopes.svyCSSUtils.StyleParser: Constructor functions should be called with the \"new\" keyword!");
