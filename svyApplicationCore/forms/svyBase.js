 ...
 * 
 * TODO: The recursive invocation through form-in-tab/split panels can cause unnecessary calls to updateUI. 
 * We should investigate ways to reduce to one call per thread, perhaps using continuations to defer to the end?
 * 
 * @author Sean
 * @since 2012.10.03
...
function updateUI(){
	...
	for(var i=0; i<elements.length; i++){											//	Iterate over elements
		...
		if(type == ELEMENT_TYPES.TABPANEL){											//	operating only on tab panels
			...
			var tabPanel = e;
			form = forms[tabPanel.getTabFormNameAt(tabPanel.tabIndex)];				//	get the selected tab form
			...
			var splitPane = e;
			form = splitPane.getLeftForm();											//	update left form UI
			if(form && form[updateUIMethod]){
				form[updateUIMethod]();
			}
			form = splitPane.getRightForm();										//	update right form UI
			if(form && form[updateUIMethod]){
				form[updateUIMethod]();
			}
		}
